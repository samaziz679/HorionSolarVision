...
export function BankEntryList({ entries }: BankEntryListProps) {
  ...
  return (
    <Table>
      ...
      <TableBody>
        {entries.map((entry) => {
          const hasReconciliations = entry.reconciliations && entry.reconciliations.length > 0
          const isFullyReconciled = entry.total_reconciled === entry.amount
          const isPartiallyReconciled = (entry.total_reconciled || 0) > 0 && !isFullyReconciled
          ...
          return (
            <TableRow key={entry.id}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="text-sm">
                        <div className="font-medium text-green-600">
                          {isFullyReconciled ? "Rapproché" : "Partiellement rapproché"}
                        </div>
                        <div className="text-muted-foreground">{entry.reconciliations.length} vente(s) liée(s)</div>
                      </div>
                    </div>
                    {isPartiallyReconciled && (
                      <Badge variant="outline" className="text-xs">
                        {entry.total_reconciled?.toLocaleString("fr-FR")} / {entry.amount.toLocaleString("fr-FR")} FCFA
                      </Badge>
                    )}
                  </div>
                ) : entry.account_type === "in" ? (
              ...
              <TableCell className="text-right">
                <span
                  className={
                    entry.account_type === "in" ? "text-green-600 font-semibold" : "text-red-600 font-semibold"
                  }
                >
                  {entry.account_type === "in" ? "+" : "-"}
